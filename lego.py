...
st.write(('My intention is to show the lego story and analyze similarities between sets'))
...
sales[['revenue', 'expenses', 'netprofit']].plot(kind='bar', stacked=True, figsize=(10, 6), ax=ax3)
# Display the plot in Streamlit
st.pyplot(fig3)

# Create a custom color map to show negative netprofit values in red
# colors = ['red' if x < 0 else 'blue' for x in sales['netprofit']]
...
sales_before_2004[['revenue', 'expenses', 'netprofit']].plot(kind='bar', stacked=True, figsize=(10, 6), ax=ax1)
...
sales_after_2004[['revenue', 'expenses', 'netprofit']].plot(kind='bar', stacked=True, figsize=(10, 6), ax=ax2)
...
ax4 = sales[['employees']].plot(kind='bar', stacked=False, figsize=(10, 6), color=colors, title='Number of Employees and Net Profit by Year', rot=45)
...
fig4, ax4 = plt.subplots()
colors = ['#4ea080', '#5eb698', '#6fccb1', '#80e2ca', '#92f9e4', '#ffffe0']
sales[['employees']].plot(kind='bar', stacked=False, figsize=(10, 6), color=colors, title='Number of Employees and Net Profit by Year', rot=45, ax=ax4)
...
y_values = [0, round(sales['emprev'].max()/2, -3), round(sales['emprev'].max()*0.75, -3)]  # Rounded y-values for horizontal lines
...
# Treemap by setnum values  / year
fig7= px.treemap(allset, path=['year','cat1','cat2','cat3'], values='set_num')
fig7.update_traces(marker=dict(line=dict(width=2, color='black')))
fig7.update_traces(hovertemplate='Sets: %{value}')
